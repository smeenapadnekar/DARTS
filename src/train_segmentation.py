...
def get_class_labels(dataset_name):
    if dataset_name.startswith("cityscapes"):
        ...
        return ['unlabeled', 'car', 'person', 'bike', 'curve', 'car_stop', 'guardrail', 'color_cone', 'bump']
        ...
        return [
            'background', 'fire_extinguisher', 'backpack', 'drill', 'rescue_randy'
        ]
    elif dataset_name == "PST2":
        return [
            'background', 'foreground'
            # 'background', 'fire_extinguisher', 'backpack', 'drill', 'rescue_randy'
        ]
    elif dataset_name == "SemanticRT":
        return[
            'background', 'car stop','bike','bicyclist','motorcycle','motorcyclist','car','tricycle','traffic light','box','pole','curve','person'
        ]
    else:
...
class LitUnsupervisedSegmenter(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ind = batch["ind"]                      # index
            img = batch["img"]                      # rgb image
            thr = batch['thr']                      # thr image
            img_aug = batch["img_aug"]              # Geo+Photo rgb img aug
            thr_aug = batch["thr_aug"]              # Geo+Photo thr img aug
            coord_aug = batch["coord_aug"]          # meshgrid
            img_pos = batch["img_pos"]              # KNN rgb img
            img_pos_th = batch["img_pos_th"]        # KNN th img
            label = batch["label"]                  # label
            label_pos = batch["label_pos"]          # KNN rgb label
            label_pos_th = batch["label_pos_th"]    # KNN thr label
            ...
            # img_pos = batch["img_pos_th"]
            # label = batch["label"]
            # # label_pos = batch["label_pos"]
            # label_pos = batch["label_pos_th"]

        feats, feats_th, code = self.net(img, thr)
        if self.cfg.correspondence_weight > 0:
            feats_pos, feats_pos_th, code_pos = self.net(img_pos,img_pos_th)
        ...
        if self.cfg.use_true_labels:
            ...
            signal_th = one_hot_feats(label + 1, self.n_classes + 1)
            signal_pos_th = one_hot_feats(label_pos + 1, self.n_classes + 1)
        else:
            signal = feats
            signal_pos = feats_pos

            signal_th = feats_pos
            signal_pos_th = feats_pos_th
        ...
        if self.cfg.correspondence_weight > 0:
            (
                ...
                pos_intra_loss_th, pos_intra_cd_th,
                pos_inter_loss_th, pos_inter_cd_th,
                neg_inter_loss_th, neg_inter_cd_th,
                pos_intra_loss_rgb_th, pos_intra_cd_rgb_th,
                pos_inter_loss_rgb_th, pos_inter_cd_rgb_th,
                neg_inter_loss_rgb_th, neg_inter_cd_rgb_th
            ) = self.contrastive_corr_loss_fn(
                signal, signal_pos,
                signal_th, signal_pos_th,
            ...
            if should_log_hist:
                ...
                self.logger.experiment.add_histogram("intra_cd_th", pos_intra_cd_th, self.global_step)
                self.logger.experiment.add_histogram("inter_cd_th", pos_inter_cd_th, self.global_step)
                self.logger.experiment.add_histogram("neg_cd_th", neg_inter_cd_th, self.global_step)
                self.logger.experiment.add_histogram("neg_cd_th", pos_inter_cd_rgb_th, self.global_step)
            neg_inter_loss = neg_inter_loss.mean()
            pos_intra_loss = pos_intra_loss.mean()
            pos_inter_loss = pos_inter_loss.mean()
            neg_inter_loss_th = neg_inter_loss_th.mean()
            pos_intra_loss_th = pos_intra_loss_th.mean()
            pos_inter_loss_th = pos_inter_loss_th.mean()
            pos_intra_loss_rgb_th = pos_intra_loss_rgb_th.mean()
            pos_inter_loss_rgb_th = pos_inter_loss_rgb_th.mean()
            neg_inter_loss_rgb_th = neg_inter_loss_rgb_th.mean()
            ...
            loss += (
                    ...
                    self.cfg.pos_inter_weight_th * pos_inter_loss_th +
                    self.cfg.pos_intra_weight_th * pos_intra_loss_th +
                    self.cfg.neg_inter_weight_th * neg_inter_loss_th
                )*0.4* self.cfg.correspondence_weight +(
                    self.cfg.pos_intra_weight_rgb_th * pos_intra_loss_rgb_th +
                    self.cfg.pos_inter_weight_rgb_th * pos_inter_loss_rgb_th +
                    self.cfg.neg_inter_weight_rgb_th * neg_inter_loss_rgb_th
                )*0.5*self.cfg.correspondence_weight
        ...
        if self.cfg.aug_alignment_weight > 0:
            orig_feats_aug, orig_feats_aug_th, orig_code_aug = self.net(img_aug,thr_aug)
        ...
        # if supervised:
        # detached_code = torch.clone(code)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            feats, feat_th, code = self.net(img,thr)
