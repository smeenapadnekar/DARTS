...
def get_feats(model, loader,mode):
    all_feats = []
...
def my_app(cfg: DictConfig) -> None:
    ...
    # dataset_names = ["cocostuff27", "cityscapes", "potsdam","MFNET","KP"]
    dataset_names = ["KP"]
    crop_types = [None]
    ...
    if cfg.arch == "dino":
        from modules import DinoFeaturizer_old, LambdaLayer
        no_ap_model = torch.nn.Sequential(
            DinoFeaturizer_old(20, cfg),  # dim doesent matter
            ...
            for dataset_name in dataset_names:
                for mode in modality:
                    ...
                    if mode == 'img':
                        ...
                        feature_cache_file = join(pytorch_data_dir, "nns", "nns_{}_{}_th_{}_{}_{}.npz".format(
                            cfg.model_type, nice_dataset_name, image_set, crop_type, res))
                    print(feature_cache_file)
                    if not os.path.exists(feature_cache_file):
                        ...
                        with torch.no_grad():
                            normed_feats = get_feats(par_model, loader, mode)
                            all_nns = []
