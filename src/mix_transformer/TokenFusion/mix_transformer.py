...
from .Token_Fusion_modules import ModuleParallel, LayerNormParallel, num_parallel, TokenExchange
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=320, patch_size=7, stride=1, in_chans=3, embed_dim=768):
        super().__init__()
...
class Token_Fusion(nn.Module):
    def __init__(self, img_size=320, patch_size=16, in_chans=3, embed_dims=[64, 128, 256, 512],
                 num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=LayerNormParallel,
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1]):
        super().__init__()
        ...
        # patch_embed
        # self.patch_embed1 = OverlapPatchEmbed(img_size=img_size, patch_size=8, stride=4, in_chans=in_chans,
        #                                       embed_dim=embed_dims[0])
        # self.patch_embed2 = OverlapPatchEmbed(img_size=img_size, patch_size=8, stride=2, in_chans=embed_dims[0],
                                            #   embed_dim=embed_dims[1])
        predictor_list = [PredictorLG(embed_dims[i]) for i in range(len(depths))]
        self.score_predictor = nn.ModuleList(predictor_list)
    ...
    @torch.jit.ignore
    def no_weight_decay(self):
        return {'pos_embed1', 'pos_embed2', 'cls_token'}  # has pos_embed may be better
    
    def forward_features(self, x):
        B = x[0].shape[0]
        outs0, outs1 = [], []
        masks = []
        H=W=int(np.sqrt(x[0].shape[1]))
        # stage 1
        # x, H, W = self.patch_embed1(x)
        for i, blk in enumerate(self.block1):
            score = self.score_predictor[0](x)
...
class mit_TF(Token_Fusion):
    def __init__(
            self, img_size=320, patch_size=8, in_chans=3, embed_dims=[768, 768], 
            num_heads=[6, 8], mlp_ratios=[4, 4], qkv_bias=True, 
            qk_scale=None, drop_rate=0, attn_drop_rate=0, 
            drop_path_rate=0.1, norm_layer=LayerNormParallel, 
            depths=[2,2], sr_ratios=[4, 1]
    ):
        super().__init__(
    ...
    #     super(mit_TF,self).__init__(
    #         patch_size=8, embed_dims=[768,768], num_heads=[6, 8], mlp_ratios=[4, 4, 4, 4],
    #         qkv_bias=True, norm_layer=LayerNormParallel, depths=[2,2], sr_ratios=[2, 1],
...
# img = Image.open("/media/vplab/OneTouch/Meena/Project/rgbt-NTSemSeg/DatasetVisualization/KP/visible/I00792.jpg")
...
# dino_res = dino.get_intermediate_layers(img.unsqueeze(0),1)
# dino_res = dino_res[0][:,1:,:]
# print(dino_res.shape)
# res = model([dino_res,dino_res])
# print(len(res))
# print(len(res[0][0]),res[0][0][1].shape)
# print(len(res[1]))