...
class GeminiTokenFusion(nn.Module):
    def __init__(
        self,
        img_size=320,
        patch_size=16,
...
class Gemini_TF(GeminiTokenFusion):
    def __init__(self,  n_heads, dpr, drop_rate, img_size=320, patch_size=8, in_chans=3, num_classes=1000, embed_dims=[768,768], num_heads=[ 4, 8], mlp_ratios=[4, 4], qkv_bias=True, qk_scale=None, attn_drop_rate=0, drop_path_rate=0, norm_layer=LayerNormParallel, depths=[6, 3], sr_ratios=[4, 1]):
        super(Gemini_TF,self).__init__(
...
# dpr=0.1
# drop_rate=0.0
# # model = mit_b5(n_heads=n_heads,dpr=dpr,drop_rate=drop_rate)
# # print(model)

# dino = torch.hub.load('facebookresearch/dino:main', 'dino_vitb8')
# model = Gemini_TF(n_heads=n_heads,dpr=dpr,drop_rate=drop_rate)
# # print(model)
