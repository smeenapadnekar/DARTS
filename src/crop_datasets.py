...
class RandomCropComputer(Dataset):
    ...
    def __init__(self, cfg, dataset_name, img_set, crop_type, crop_ratio,mode):
        ...
        if mode == 'img':
            ...
            self.save_dir = join(
                cfg.pytorch_data_dir, "cropped", "{}_th_{}_crop_{}".format(dataset_name, crop_type, crop_ratio))
    ...
    def __getitem__(self, item):
        ...
        for crop_num, (img, label) in enumerate(zip(imgs, labels)):
            ...
            # For potsdam and cityscapes, MFNET
...
def my_app(cfg: DictConfig) -> None:
    ...
    # dataset_names = ["cityscapes", "cocostuff27","potsdam","MFNET","KP"]
    dataset_names = ["SemanticRT"]
    modality = ["img","thermal"]
    img_sets = ["train", "val"]
    crop_types = ["five"]
    crop_ratios = [.5]
            ...
            for dataset_name in dataset_names:
                for mode in modality:
                    for img_set in img_sets:
                        dataset = RandomCropComputer(cfg, dataset_name, img_set, crop_type, crop_ratio,mode)
