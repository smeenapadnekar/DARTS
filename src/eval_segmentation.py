...
def my_app(cfg: DictConfig) -> None:
    ...
    os.makedirs(join(result_dir, "thr"), exist_ok=True)
    ...
    for model_path in cfg.model_paths:
        ...
        elif model.cfg.dataset_name == "MFNET":
            all_good_images = [19, 54, 67, 66, 65, 75, 77, 76, 124]
            all_good_images = [i for i in range(len(test_dataset))] 
        elif model.cfg.dataset_name == "KP":
            all_good_images = [19, 54, 67, 66, 65, 75, 77, 76, 124]
            all_good_images = [i for i in range(len(test_dataset))]
        elif model.cfg.dataset_name == "PST":
            all_good_images = [19, 54, 67, 66, 65, 75, 77, 76, 124]
            all_good_images = [i for i in range(len(test_dataset))]
        elif model.cfg.dataset_name == "PST2":
            all_good_images = [19, 54, 67, 66, 65, 75, 77, 76, 124]
            all_good_images = [i for i in range(len(test_dataset))]
        elif model.cfg.dataset_name == "SemanticRT":
            all_good_images = [19, 54, 67, 66, 65, 75, 77, 76, 124]
            all_good_images = [i for i in range(len(test_dataset))]
        else:
        ...
        with Pool(cfg.num_workers + 5) as pool:
            for i, batch in enumerate(tqdm(test_loader)):
                with torch.no_grad():
                    ...
                    feats, feats_th, code1 = par_model(img,thr)
                    torch.cuda.synchronize()
                    end_time = time.time()
                    feats, feats_th, code2 = par_model(img.flip(dims=[3]),thr.flip(dims=[3]))
                    ...
                    #         saved_data["thr"].append(thr.cpu()[offset].unsqueeze(0))
                    #         if run_picie:
                    #             saved_data["picie_preds"].append(picie_preds.cpu()[offset].unsqueeze(0))

                    # print(img_num.shape,linear_preds.shape,cluster_preds.shape,label.shape,img.shape)
                    for i in range(len(img_num)):
                        plot_img = (prep_for_plot(img[i]) * 255).numpy().astype(np.uint8)
                        plot_label = (model.label_cmap[label[i].squeeze(0).cpu()]).astype(np.uint8)
                        # plot_thr = (prep_for_plot(thr[i])).numpy().astype(np.uint8)
                        plot_thr = thr[i].permute(1,2,0).cpu().numpy().astype(np.uint8)
                        Image.fromarray(plot_img).save(join(join(result_dir, "img", str(i) + ".jpg")))
                        Image.fromarray(plot_thr).save(join(join(result_dir, "thr", str(i) + ".jpg")))
                        Image.fromarray(plot_label).save(join(join(result_dir, "label", str(i) + ".png")))
                        plot_cluster = (model.label_cmap[cluster_preds[i].cpu()]).astype(np.uint8)
                        Image.fromarray(plot_cluster).save(join(join(result_dir, "cluster", str(i) + ".png")))
                        plot_linear = (model.label_cmap[linear_preds[i].cpu()]).astype(np.uint8)
                        Image.fromarray(plot_linear).save(join(join(result_dir, "linear", str(i) + ".png")))
