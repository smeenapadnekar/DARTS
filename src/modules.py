...
from mix_transformer.GeminiFusion.mix_transformer import Gemini_TF
# from mix_transformer.TokenFusion.mix_transformer import mit_TF
from mix_transformer.LinearFusion.mix_transformer import linear_tf
...
class DinoFeaturizer(nn.Module):
    ...
    def __init__(self, dim, cfg):
        ...
        self.alpha_thr = self.alpha_param()
        self.alpha_img = self.alpha_param()
        # self.alpha_thr_fuse = self.alpha_param()
        # self.alpha_img_fuse = self.alpha_param()
        ...
        # Cross Attention
        # self.rgb_to_thermal_attention = nn.MultiheadAttention(self.n_feats, num_heads=3)
        # self.thermal_to_rgb_attention = nn.MultiheadAttention(self.n_feats, num_heads=3)
        self.norm = nn.LayerNorm(self.n_feats)

        self.token_fusion = Gemini_TF(
            n_heads=6,                          
            dpr=0.1,
            drop_rate=0,
            patch_size=self.patch_size,
            embed_dims=[self.n_feats],
            num_heads=[6],
            mlp_ratios=[4], 
            depths=[1],
            sr_ratios=[4]
        ...
        #     drop_rate=0,
        #     patch_size=self.patch_size,
        #     embed_dims=[self.n_feats,self.n_feats],
        #     num_heads=[6,6],
        #     mlp_ratios=[4,4],
        #     depths=[1,1],
        #     sr_ratios=[4,4]
        # )
        # self.linear_token_fusion = linear_tf(
        #     ratio=0.8,
        #     drop_rate=0,
        #     drop_path_rate=0.1,
        #     patch_size=self.patch_size,
        ...
        #     sr_ratios=[4]
        # )
        # self.smooth_kernel = nn.Conv2d(self.n_feats, self.n_feats, kernel_size=3, padding=3 // 2, bias=False)
        # # self.smooth_kernel.weight.data = torch.tensor([[[[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]]]], dtype=torch.float32)
        # nn.init.constant_(self.smooth_kernel.weight, 1 / (3 * 3))
        # self.smoothness_regularizer = self.smoothness_regulizer_net()
    ...
    def smoothness_regulizer_net(self):
    ...
    def alpha_param(self):
        return nn.Parameter(torch.rand((self.model.embed_dim)))
    
    def forward(self, img, thr, n=1, return_class_feat=False):
        self.model.eval()
        with torch.no_grad():
            ...
            # get activation for thermal
            feat_th, attn_th, qkv_th = self.model.get_intermediate_feat(thr, n=n)
            feat_th, attn_th, qkv_th = feat_th[0], attn_th[0], qkv_th[0]
            ...
            if self.feat_type == "feat":
                ...
                thr_feat = feat_th[:, 1:, :]#.reshape(feat_th.shape[0], feat_h, feat_w, -1).permute(0, 3, 1, 2)
                ...
                thr_k = qkv[1, :, :, 1:, :].reshape(feat_th.shape[0], 6, feat_h, feat_w, -1)
                B, H, I, J, D = thr_k.shape
                thr_feat = thr_k.permute(0, 1, 4, 2, 3).reshape(B, H * D, I, J)
            else:
                raise ValueError("Unknown feat type:{}".format(self.feat_type))

            if return_class_feat:
                return feat[:, :1, :].reshape(feat.shape[0], 1, 1, -1).permute(0, 3, 1, 2), feat_th[:, :1, :].reshape(feat_th.shape[0], 1, 1, -1).permute(0, 3, 1, 2)

        # Token Fusion
        # fused_feat = self.linear_token_fusion([image_feat,thr_feat])
        # img_token_fused = fused_feat[0][-1]#.reshape(feat.shape[0], feat_h, feat_w, -1)
        # thr_token_fused = fused_feat[1][-1]#.reshape(feat.shape[0], feat_h, feat_w, -1)
        # fused_feat = self.token_fusion([img_token_fused,thr_token_fused])
        fused_feat = self.token_fusion([image_feat,thr_feat])
        img_token_fused = fused_feat[0][-1].reshape(feat.shape[0], feat_h, feat_w, -1)
        thr_token_fused = fused_feat[1][-1].reshape(feat.shape[0], feat_h, feat_w, -1)
        # print(img_token_fused.shape,thr_token_fused.shape, image_feat.shape, thr_feat.shape)
        image_feat = image_feat.reshape(feat.shape[0], feat_h, feat_w, -1)
        thr_feat = thr_feat.reshape(feat.shape[0], feat_h, feat_w, -1)
        fused_feat = torch.add(self.alpha_img*img_token_fused,self.alpha_thr*thr_token_fused).permute(0,3,1,2)
        # fused_feat_feat = torch.add(self.alpha_img*image_feat,self.alpha_thr*thr_feat).permute(0,3,1,2)
        # fused_feat = torch.add(fused_feat_fuse,fused_feat_feat)
        # Cross Attn
        # rgb_fuse,_ = self.rgb_to_thermal_attention(image_feat,thr_feat,thr_feat)
        # thr_fuse,_ = self.thermal_to_rgb_attention(thr_feat,image_feat,image_feat)
        # rgb_fuse = self.norm(rgb_fuse+image_feat).reshape(feat.shape[0], feat_h, feat_w, -1)
        # thr_fuse = self.norm(thr_fuse+thr_feat).reshape(feat.shape[0], feat_h, feat_w, -1)
        # fused_feat = torch.add(self.alpha_img*rgb_fuse,self.alpha_thr*thr_fuse).permute(0, 3, 1, 2)
        # smoothness_map = self.smooth_kernel(thr_feat).permute(0,2,3,1)
        # smoothed_rgb_features = self.smoothness_regularizer(image_feat).permute(0,2,3,1)
        # # fused_feat = smoothed_rgb_features * smoothness_map
        
        # fused_feat = torch.add(self.alpha_img*smoothed_rgb_features,self.alpha_thr*smoothness_map).permute(0, 3, 1, 2)
        image_feat = image_feat.permute(0,3,1,2)
        thr_feat = thr_feat.permute(0,3,1,2)
        # image_feat = fused_feat
        if self.proj_type is not None:
            code = self.cluster1(self.dropout(fused_feat))
            if self.proj_type == "nonlinear":
                code += self.cluster2(self.dropout(fused_feat))
        else:
            code = fused_feat
        
        if self.cfg.dropout:
            return self.dropout(image_feat),self.dropout(thr_feat), code
        else:
            return image_feat, thr_feat, code

class DinoFeaturizer_old(nn.Module):
...
class ContrastiveCorrelationLoss(nn.Module):
    ...
    def forward(self,
                ...
                orig_feats_th: torch.Tensor, orig_feats_pos_th: torch.Tensor,
        ...
        feats_th = sample(orig_feats_th, coords1)
        code = sample(orig_code, coords1)

        feats_pos = sample(orig_feats_pos, coords2)
        feats_pos_th = sample(orig_feats_pos_th, coords2)
        ...
        pos_intra_loss_th, pos_intra_cd_th = self.helper(
            feats_th, feats_th, code, code, self.cfg.pos_intra_shift_th)
        pos_inter_loss_th, pos_inter_cd_th = self.helper(
            feats_th, feats_pos_th, code, code_pos, self.cfg.pos_inter_shift_th)

        # Cross-modal Feature Correlation Loss
        pos_intra_loss_rgb_th, pos_intra_cd_rgb_th = self.helper(
            (feats+feats_th)/2,(feats+feats_th)/2, code, code, self.cfg.pos_intra_shift_rgb_th)
        
        pos_inter_loss_rgb_th, pos_inter_cd_rgb_th = self.helper(
            (feats+feats_th)/2,(feats_pos+feats_pos_th)/2, code, code_pos, self.cfg.pos_inter_shift_rgb_th)
        ...
        neg_cds = []
        for i in range(self.cfg.neg_samples):
            perm_neg = super_perm(orig_feats_th.shape[0], orig_feats.device)
            feats_neg = sample(orig_feats_th[perm_neg], coords2)
            code_neg = sample(orig_code[perm_neg], coords2)
            neg_inter_loss_th, neg_inter_cd_th = self.helper(
                feats_th, feats_neg, code, code_neg, self.cfg.neg_inter_shift_th)
            neg_losses.append(neg_inter_loss_th)
            neg_cds.append(neg_inter_cd_th)
        neg_inter_loss_th = torch.cat(neg_losses, axis=0)
        neg_inter_cd_th = torch.cat(neg_cds, axis=0)
        ...
        for i in range(self.cfg.neg_samples):
            ...
            perm_neg_th = super_perm(orig_feats_th.shape[0], orig_feats.device)
            feats_neg_th = sample(orig_feats_th[perm_neg], coords2)
            code_neg = sample(orig_code[perm_neg], coords2)
            neg_inter_loss_th, neg_inter_cd_th = self.helper(
                (feats+feats_th)/2, (feats_neg+feats_neg_th)/2, code, code_neg, self.cfg.neg_inter_shift_rgb_th)
            neg_losses.append(neg_inter_loss_th)
            neg_cds.append(neg_inter_cd_th)
        neg_inter_loss_rgb_th = torch.cat(neg_losses, axis=0)
        neg_inter_cd_rgb_th = torch.cat(neg_cds, axis=0)
        ...
        return (pos_intra_loss.mean(),
                ...
                neg_inter_loss,
                neg_inter_cd,
                pos_intra_loss_th.mean(),
                pos_intra_cd_th,
                pos_inter_loss_th.mean(),
                pos_inter_cd_th,
                neg_inter_loss_th,
                neg_inter_cd_th,
                pos_intra_loss_rgb_th,
                pos_intra_cd_rgb_th,
                pos_inter_loss_rgb_th,
                pos_inter_cd_rgb_th,
                neg_inter_loss_rgb_th,
                neg_inter_cd_rgb_th
                )
